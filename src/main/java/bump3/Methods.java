...
public class Methods {
	...
	public static void p(String txt) {
		// same as println, checks for quiet
		if (Main.QUIET == false)
	...
	public static void pr(String txt) {
		// same as print, checks for quiet
		if (Main.QUIET == false)
	...
	public static boolean GuiStop() {
		// returns true only if we are in GUI mode and GuiSearch.STOP is true
		if (Main.GUI)
			return GuiSearch.STOP;
		else
			return false;
	}
	...
	public static int searchSong(String artist, String title) {
		...
		if (!Main.ENGINE.equals("")) {
			...
			if (s.equalsIgnoreCase("google") || 
				s.equalsIgnoreCase("google.com"))
				return Google.search(artist, title);
			
			else if (s.equalsIgnoreCase("dreammedia") || 
				 s.equalsIgnoreCase("dreammedia.ru"))
				return Dreammedia.search(artist, title);
			
			else if (s.equalsIgnoreCase("findmp3s") || 
				 s.equalsIgnoreCase("findmp3s.com"))
				return Findmp3s.search(artist, title);
			
			else if (s.equalsIgnoreCase("oth") || 
				 s.equalsIgnoreCase("oth.net"))
				return Oth.search(artist, title);
			
			else if (s.equalsIgnoreCase("seekasong") || 
				 s.equalsIgnoreCase("seekasong.com"))
				return Seekasong.search(artist, title);
			
			else if (s.equalsIgnoreCase("espew") || 
				 s.equalsIgnoreCase("espew.net"))
				return Espew.search(artist, title);
				
			else if (s.equalsIgnoreCase("dilandau") || 
				 s.equalsIgnoreCase("dilandau.com"))
				return Dilandau.search(artist, title);
				
			else if (s.equalsIgnoreCase("pepperoni") || 
				 s.equalsIgnoreCase("pepperoni.com") || 
				 s.equalsIgnoreCase("pep"))
				return Pepperoni.search(artist, title);
				
			else if (s.equalsIgnoreCase("emp3world") || 
				 s.equalsIgnoreCase("emp3world.com") || 
				 s.equalsIgnoreCase("emp3"))
				return Emp3World.search(artist, title);
				
			else if (s.equalsIgnoreCase("mp3skull") || 
				 s.equalsIgnoreCase("mp3skull.com"))
				return Mp3skull.search(artist, title);
			
			else if (s.equalsIgnoreCase("mp3center") || 
				 s.equalsIgnoreCase("mp3-center") || 
				 s.equalsIgnoreCase("mp3-center.org"))
				return Mp3center.search(artist, title);
			
			else if (s.equalsIgnoreCase("downloads.nl") ||
				 s.equalsIgnoreCase("downloadsnl") ||
				 s.equalsIgnoreCase("downloads"))
				return Downloadsnl.search(artist, title);
			
			else if (s.equalsIgnoreCase("4shared") ||
				 s.equalsIgnoreCase("4shared.com"))
				return FourShared.search(artist, title);
			
			else if (s.equalsIgnoreCase("prostopleer") ||
				 s.equalsIgnoreCase("prostopleer.com"))
				return Prostopleer.search(artist, title);
			
			else if (s.equalsIgnoreCase("myzuka") ||
				 s.equalsIgnoreCase("myzuka.ru"))
				return Myzuka.search(artist, title);
			
			else if (s.equalsIgnoreCase("plugin")) {
	...
	public static int searchAlbum(String artist, String album) {
		// search sites for entire albums... this could take a while...
		// probably never gonna implement it!
		return 0;
	}
	...
	*/
	...
	public static void loadSettings() {
		...
		try {
			...
			// System.out.println("READING FROM \'" + dir+"bump3.ini\'");
			
			f = new Scanner(new FileReader(dir+"bump3.ini"));
		} catch (FileNotFoundException fnfe) {
			// not found, no settings to load.
			return;
		}
		...
		while (f.hasNext()) {
			...
			if (p.equals("min_filesize"))
				Main.MIN_FILESIZE = Integer.parseInt(v);
				
			else if (p.equals("save_dir")) {
				if (fileExists(v))
					Main.SAVE_DIR = v;
			}
			
			else if (p.equals("autoplay"))
				Main.AUTOPLAY = Boolean.parseBoolean(v);
				
			else if (p.equals("autoplay_wait"))
				Main.AUTOPLAY_WAIT = Integer.parseInt(v);
				
			else if (p.equals("playstring"))
				Main.PLAYSTRING = v;
				
			else if (p.equals("connect_timeout"))
				Main.CONNECT_TIMEOUT = Integer.parseInt(v);
				
			else if (p.equals("read_timeout"))
				Main.READ_TIMEOUT = Integer.parseInt(v);
			
			else if (p.equals("recent_files")) {
				Main.RECENTS = v.split(";");
				String temp = "";
				for (int i = 0; i < Main.RECENTS.length; i++) {
					if (fileExists(Main.RECENTS[i])) {
						temp += Main.RECENTS[i];
						if (i + 1 != Main.RECENTS.length)
							temp += ";";
					}
				}
				...
				if (temp.equals(""))
					...
					Main.RECENTS = temp.split(";");
				
			} else if (p.equals("engines")) {
				//Main.ENGINES = new boolean[]{true,true,true,true,true,true,true,true,true,true,true,false};
				String[] temp = v.split(",");
				for (int i = 0; i < temp.length; i++) {
					if (temp[i].equals("0"))
						Main.ENGINES[i] = false;
				}
				
			} else if (p.equals("theme"))
				Main.THEME = Boolean.parseBoolean(v);
				
			else if (p.equals("agreed"))
				Main.AGREED = Boolean.parseBoolean(v);
			
			else if (p.equals("nospaces"))
				Main.NOSPACES = Boolean.parseBoolean(v);
			
			else if (p.equals("msg")) {
				// first load after an upgrade
				// check for upgrayedd.bat and upgrayedd.vbs on windows
				// ...or upgrayedd.sh on Linux
				try {
					if (Main.OS.startsWith("Windows")) {
						if (fileExists(Main.PROG_DIR + "upgrayedd.vbs"))
							Runtime.getRuntime().exec("del \"" + Main.PROG_DIR + "upgrayedd.vbs\"");
						if (fileExists(Main.PROG_DIR + "upgrayedd.bat"))
							Runtime.getRuntime().exec("del \"" + Main.PROG_DIR + "upgrayedd.bat\"");
							
					} else {
						// probably won't need to delete the .sh ... sh should have removed itself
						/*if (fileExists(Main.PROG_DIR + "upgrayedd.sh"))
							Runtime.getRuntime().exec("rm \"" + Main.PROG_DIR + "upgrayedd.sh\"");*/
					}
			...
			} else if (p.equals("nodupes"))
				Main.NODUPES = Boolean.parseBoolean(v);
	...
	public static void saveSettings(String newSetting) {
		...
		try {
			String dir = Main.PROG_DIR;
			// System.out.println("WRITING TO   \'" + dir+"bump3.ini\'");
			...
			if (Main.RECENTS != null)
				f.write("recent_files="   +join(Main.RECENTS,";")  +"\n");
			
			if (Main.theGUI != null)
				f.write("engines="        +join(Gui.mnuEngines,",")+"\n");
			
			if (Main.DOWNLOADED != null && Main.NODUPES)
				f.write("downloaded_urls="   +join(Main.DOWNLOADED,";")  +"\n");
	...
	public static boolean hasBeenDownloaded(String url) {
		for (int i = 0; i < Main.DOWNLOADED.length; i++) {
			// System.out.println("COMPARING: " + Main.DOWNLOADED[i] + " TO " + url);
			if (url.equals(Main.DOWNLOADED[i]))
				return true;
		}
    ...
    public static void playSong(String file) {
    	// converts the command-line into 
    	String [] cmd = Main.PLAYSTRING.split(" ");
    	try {
			
    		for (int i = 0; i < cmd.length; i++) {
    			if (cmd[i].equalsIgnoreCase("%s"))
    				cmd[i] = file;
    		}
	...
	public static void openUrl(String url){
		// launch web browser towards specified 'url'
		...
		try{
			...
			//Block for Mac OS
				...
				// sucks to be you!
				// i don't want to have outdated or deprecated methodologies in my program
			...
			//Block for UNIX Platform
				...
				for (int count = 0; count < browsers.length && browser == null; count++)
					if (runtime.exec(
						...
						).waitFor() == 0) { // app exists (which output something)
	...
	public static boolean upgrayedd(String url) {
		...
		if (Main.OS.startsWith("Windows")) {
			// create a batch file to upgrade bump3
			try {
				...
				fw.write(
				"@echo off" + newline +
				"del \"" + Main.PROG_DIR + Main.FILE_NAME + "\"" + newline +
				"move \"" + Main.PROG_DIR + "bump3_new.jar\" \"" + Main.PROG_DIR + Main.FILE_NAME + "\"" + newline +
				"java -jar \"" + Main.PROG_DIR + Main.FILE_NAME + "\"" + newline +
				"del \"" + Main.PROG_DIR + "upgrayedd.vbs" + newline + 
				"del \"" + Main.PROG_DIR + "upgrayedd.bat" + newline);
				fw.close();
			...
			// create a shell script to upgrade bump3
			try {
				FileWriter fw = new FileWriter(Main.PROG_DIR + "upgrayedd.sh");
				fw.write(
				"#/bin/sh" + newline + 
				"rm \"" + Main.PROG_DIR + Main.FILE_NAME + "\"" + newline +
				"mv \"" + Main.PROG_DIR + "bump3_new.jar\" \"" + Main.PROG_DIR + Main.FILE_NAME + "\"" + newline +
				"java -jar \"" + Main.PROG_DIR + Main.FILE_NAME + "\"" + newline +
				"rm \"" + Main.PROG_DIR + "upgrayedd.sh\"" + newline);
				fw.close();
				...
				if (!System.getProperty("user.name").equals("root")) {
					// can't chmod as a non-root
					JOptionPane.showMessageDialog(
						null,
						"the newest version of BuMP3 has been downloaded as \n" +
						...
						"if BuMP3 does not reload, simply move bump3_new.jar in place of " + Main.FILE_NAME + "!",
						"BuMP3 - Linux is iffy",
	...
	public static void launchDirectory(String dir) {
		...
		if (Main.OS.startsWith("Windows")) {
			...
			String[] viewers = new String[]{
					...
					"emelfm", "rox-filer", "pcmanfm", "xnc", "mc"};
			try {
