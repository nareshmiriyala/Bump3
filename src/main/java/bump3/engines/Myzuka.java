...
public class Myzuka {
	...
	public static int search(String theArtist, String theTitle) {
		...
		String theURL = "http://myzuka.ru/Search?page=1&type=artists&searchText=" +
			...
			while (!page2.equals("")) {
				for (String chunk : between(page2, " href=\"/Song", "/a>")) {
					...
					Methods.pv(Main.GR + "[+] Comparing " +
							...
							Main.G + maintitle + Main.GR + ""); 
					
					//if (!maintitle.equals(title)) continue;
					...
					if (size == -2) {
						...
						p(Main.R+"Filesize is too small (" + size + " < " +
							Main.GR+Main.MIN_FILESIZE+Main.R+ ")");
					}
